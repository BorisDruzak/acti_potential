...
class AddDocumentDialog(QDialog):
    ...
    def __init__(self, db_manager: Any, document_handler: WordDocumentHandler, parent=None, file_path=None):
    ...
    def _set_empty_reg_date(self):
        """Сделать дату регистрации пустой (специальное значение)."""
        if not hasattr(self, 'metadata_editor') or not hasattr(self.metadata_editor, 'reg_date_field'):
            return
        
        reg_date_field = self.metadata_editor.reg_date_field
        if hasattr(self.metadata_editor, '_set_date_field_empty'):
            self.metadata_editor._set_date_field_empty(reg_date_field)
            return
        empty_date = QDate(1900, 1, 1)
        reg_date_field.setMinimumDate(empty_date)
        reg_date_field.setSpecialValueText("")
        reg_date_field.setDate(empty_date)
    
    def _is_empty_reg_date(self, qdate):
        """Проверка, что дата регистрации не выбрана."""
        if qdate is None or not hasattr(qdate, 'isValid') or not qdate.isValid():
            return True
        if not hasattr(self, 'metadata_editor') or not hasattr(self.metadata_editor, 'reg_date_field'):
            return False
        
        return qdate == self.metadata_editor.reg_date_field.minimumDate()
    ...
    def _get_target_folder_for_reg_date(self, reg_date_qobject, has_reg_date):
        """Выбрать папку хранения файла по дате или without_date."""
        if has_reg_date:
            return self.db_manager.get_files_path(reg_date_qobject.year(), reg_date_qobject.month())
    ...
    def create_expanded_metadata_section(self):
        ...
        self._ensure_main_metadata_group_open()
    ...
    def _hide_form_row_for_widget(self, target_widget):
        """Скрыть строку поля в QFormLayout без скрытия всего контейнера."""
        if not target_widget:
            return False

        for candidate in self.metadata_editor.findChildren(QWidget):
            form_layout = getattr(candidate, "content_layout", None)
            ...
            for row in range(form_layout.rowCount()):
                ...
                if field_widget == target_widget or label_widget == target_widget:
                    if label_widget:
                        label_widget.hide()
                    if field_widget:
                        field_widget.hide()
    ...
    def _ensure_main_metadata_group_open(self):
        """Гарантированно раскрыть блок 'Основные данные документа'."""
        for candidate in self.metadata_editor.findChildren(QWidget):
            title_attr = getattr(candidate, "title", "")
            if callable(title_attr):
                ...
                title_text = str(title_attr) if title_attr else ""

            if (
                "Основные данные документа" not in title_text
                and "Основные данные нового документа" not in title_text
            ):
                continue

            if hasattr(candidate, "collapsed"):
                candidate.collapsed = False
            if hasattr(candidate, "content_widget") and candidate.content_widget:
                candidate.content_widget.setVisible(True)
            if hasattr(candidate, "toggle_btn") and candidate.toggle_btn:
                candidate.toggle_btn.setText("▼")
                candidate.toggle_btn.update()
            break
    ...
    def hide_unnecessary_elements(self):
        ...
        for line_edit in self.metadata_editor.findChildren(QLineEdit):
            ...
            'название документа' in placeholder:
                if not self._hide_form_row_for_widget(line_edit):
                    line_edit.hide()
    ...
    def add_document(self):
        ...
        reg_date_qobject = metadata.get('reg_date')
        has_reg_date = not self._is_empty_reg_date(reg_date_qobject)
        reg_date_string = reg_date_qobject.toString("yyyy-MM-dd") if has_reg_date else None
        ...
        if selected_file and os.path.exists(selected_file):
            ...
            try:
                ...
                # Если дата не выбрана, используем текущий месяц только для файлового пути.
                if has_reg_date:
                ...
                if has_reg_date:
                    ...
                    print("📅 Дата регистрации не указана -> папка without_date")
                
                # Получаем путь к папке для файлов из DatabaseManager
                target_folder = self._get_target_folder_for_reg_date(reg_date_qobject, has_reg_date)
            ...
            # Для многопользовательского режима используем UUID-суффикс при коллизии.
            if os.path.exists(new_file_name):
                name_without_ext, ext = os.path.splitext(original_filename)
                suffix = uuid.uuid4().hex[:8]
                new_file_name = os.path.join(target_folder, f"{name_without_ext}_{suffix}{ext}")
                print(f"⚠️ Файл существует, используем уникальное имя: {os.path.basename(new_file_name)}")
            ...
            try:
                ...
                if hasattr(self.db_manager, "make_relative_document_path"):
                    relative_path = self.db_manager.make_relative_document_path(new_file_name)
                else:
                    relative_path = os.path.relpath(new_file_name, self.db_manager.db_folder).replace('\\', '/')
