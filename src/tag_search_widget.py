...
class FieldSelectorPopup(QFrame):
    ...
    def setup_ui(self):
        ...
        self.kind_btn = self.create_field_button("📚 Вид", "document_kind")
        self.buttons.append(self.kind_btn)
        self.additional_layout.addWidget(self.kind_btn)
...
class TagInputField(QWidget):
    ...
    def _sync_calendar_with_text(self):
        ...
        if is_valid:
            try:
                dt = parse_flexible_datetime(formatted_date)
...
class SimpleTagSearchWidget(QWidget):
    ...
    def __init__(self, db_manager=None, parent=None):
        ...
        self.REFERENCE_FIELDS = {
            ...
            'Вид': {
                'db_field': 'document_kind',
                'table': 'ref_document_kinds'
            },
            'Статус': {
                'db_field': 'status',
                'table': 'ref_status'
    ...
    def perform_search(self):
        ...
        search_data = self.build_search_filters(tags)
    ...
    def _on_inline_date_selected(self, sql_date: str):
        ...
        try:
            dt = parse_flexible_datetime(sql_date)
            display_date = format_date_for_display(sql_date)
        except:
    ...
    def on_field_selected(self, field_name: str, db_field: str):
        ...
        for btn in [self.field_popup.status_btn, self.field_popup.type_btn, self.field_popup.kind_btn,
    ...
    def on_date_selected(self, field_name: str, date_str: str, db_field: str):
        ...
        if db_field:
            try:
                date_obj = parse_flexible_datetime(date_str)
    ...
    def build_search_filters(self, tags: List[tuple]) -> dict:
        """Build the shared paginated-table filter payload from search tags."""
        filters = {}
        
        for _field_name, db_field, value in tags:
            value = str(value or "").strip()
            if not value:
                continue
            
            if db_field == 'reg_date_from':
                filters['date_from'] = value
            elif db_field == 'reg_date_to':
                filters['date_to'] = value
            elif db_field == 'reg_date_exact':
                filters['date_exact'] = value
            elif db_field in {
                'reg_number', 'title', 'number', 'status', 'type',
                'document_kind', 'executor', 'theme',
            }:
    ...
    def build_sql_query(self, tags: List[tuple]) -> dict:
        ...
        for field_name, db_field, value in tags:
            if 'date' in db_field:
                if 'from' in db_field:
                    conditions.append("date(d.reg_date) >= date(?)")
                    params.append(value)
                elif 'to' in db_field:
                    conditions.append("date(d.reg_date) <= date(?)")
                    params.append(value)
                ...
                if db_field in ('reg_number', 'title', 'number'):
                    ...
                    params.append(f"%{value}%")
                elif db_field == 'document_kind':
                    conditions.append("dk.name LIKE ?")
                    params.append(f"%{value}%")
        ...
        base_query = """
            SELECT 
                ...
                COALESCE(dk.name, 'Не указан') as document_kind,
