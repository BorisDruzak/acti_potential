...
try:
    ...
    # Не ломаем запуск всего приложения: модуль экспорта может быть недоступен
    # в смешанной/битой сборке (например новый exe + старый _internal).
    Workbook = None
...
class ExportDialog(QDialog):
    ...
    def _is_sqlserver_backend(self) -> bool:
        return getattr(self.db_manager, "backend", "sqlite") == "sqlserver"

    def _default_export_root(self) -> str:
        if self._is_sqlserver_backend():
            return getattr(self.db_manager, "db_folder", os.getcwd())
    ...
    def _build_sqlserver_export_payload(self) -> Dict:
        return {
            "backend": "sqlserver",
            "database": getattr(self.db_manager, "db_path", ""),
            "files_root": getattr(self.db_manager, "files_base_path", ""),
            "exported_at": datetime.now().isoformat(timespec="seconds"),
            "statistics": self.db_manager.get_documents_statistics(),
            "documents": self.db_manager.get_documents_for_period(),
        }

    def _write_sqlserver_export_manifest(self, export_folder: str, include_files: bool) -> Dict[str, str]:
        payload = self._build_sqlserver_export_payload()
        manifest_path = os.path.join(export_folder, "sqlserver_metadata.json")
...
Источник: {payload['database']}
Корень файлов: {payload['files_root']}

СОДЕРЖИМОЕ:
-----------
- sqlserver_metadata.json - метаданные документов и статистика
- files/ - папка документов (если включена)
...
Серверная резервная копия SQL Server выполняется на сервере отдельно.
""".strip()
        ...
        return {"manifest_path": manifest_path, "readme_path": readme_path}

    def _archive_base_name(self) -> str:
        if self._is_sqlserver_backend():
            return getattr(self.db_manager, "database", None) or getattr(self.db_manager, "profile_name", "sqlserver")

        db_path = getattr(self.db_manager, "db_path", "")
        if db_path:
            return os.path.basename(os.path.dirname(db_path))
        return getattr(self.db_manager, "profile_name", "database")

    def _create_sqlserver_files_archive(self) -> None:
        db_name = self._archive_base_name()
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        archive_name = f"{db_name}_{timestamp}.tar.gz"

        save_folder = QFileDialog.getExistingDirectory(
            self,
            "Выберите папку для сохранения архива файлов",
            self._default_export_root(),
        ...
        reply = QMessageBox.question(
            ...
            f"Создать архив файлов для профиля SQL Server?\n\n"
            f"Файл: {archive_name}\n"
            f"Путь: {save_folder}\n\n"
            ...
            f"Серверная база SQL Server в архив не включается.",
        ...
        progress.setWindowTitle("📦 Архивация файлов")
        progress.setMinimumDuration(0)
        progress.show()

        try:
            with tempfile.TemporaryDirectory(prefix="acti_sqlserver_backup_") as staging_dir:
                progress.setValue(10)
                progress.setLabelText("Подготовка метаданных...")
                self._write_sqlserver_export_manifest(
                    staging_dir,
                    include_files=self.backup_include_files.isChecked(),
                )

                if self.backup_include_exe.isChecked():
                    progress.setValue(35)
                    progress.setLabelText("Добавление исполняемого файла...")
                    app_dir = os.path.dirname(os.path.dirname(getattr(self.db_manager, "db_folder", staging_dir)))
                    exe_candidates = [
                    ...
                    for exe_path in exe_candidates:
                        if os.path.exists(exe_path):
                            shutil.copy2(exe_path, os.path.join(staging_dir, os.path.basename(exe_path)))
                            break
                ...
                for root, _, files in os.walk(staging_dir):
                    for filename in files:
                        ...
                        staged_files.append((full_path, arcname))

                if not staged_files:
                    raise RuntimeError("Не найдено файлов для архивации")

                progress.setValue(50)
                progress.setLabelText("Создание архива...")
                with tarfile.open(archive_path, "w:gz") as tar:
                    total_files = len(staged_files)
                    for index, (filepath, arcname) in enumerate(staged_files, start=1):
                        if progress.wasCanceled():
                            if os.path.exists(archive_path):
                                os.remove(archive_path)
                            return
                        tar.add(filepath, arcname=arcname)
                        progress_value = 50 + int((index / total_files) * 45)
                        progress.setValue(progress_value)
                        progress.setLabelText(f"Архивирование: {index}/{total_files}")
            ...
            QMessageBox.information(
                self,
                "✅ Архивация завершена",
                f"Архив файлов успешно создан:\n\n"
                f"📁 Файл: {archive_name}\n"
                f"📊 Размер: {archive_size:.2f} МБ\n"
                f"📂 Папка: {save_folder}\n\n"
                f"База SQL Server в архив не включалась."
            )
            self.accept()
    ...
    def init_ui(self):
        ...
        if False and self._is_sqlserver_backend():
            title = QLabel("📦 Архивация файлов")
            ...
            desc = QLabel(
                ...
                Для профиля <b>SQL Server</b> архивация самой базы данных из клиента отключена.<br>
                Серверная резервная копия SQL Server выполняется отдельно на сервере.<br>
                <br>
                Здесь доступна только архивация файлов документов и служебных метаданных клиента.<br>
                Формат архива: <b>.tar.gz</b>.
                </p>
                """
            )
            ...
            server_group = QGroupBox("🗄️ База данных SQL Server")
            server_group.setStyleSheet(AppStyles.group_box())
            server_layout = QVBoxLayout()
            server_label = QLabel(
                f"""
                <p style='color: #b71c1c; font-weight: bold;'>
                Архивация серверной базы из клиента недоступна.
                </p>
                <p>
                Сервер: {getattr(self.db_manager, 'server', '-')}<br>
                База: {getattr(self.db_manager, 'database', '-')}
                </p>
                """
            )
            server_label.setStyleSheet(AppStyles.lable())
            server_label.setTextFormat(Qt.RichText)
            server_layout.addWidget(server_label)
            server_group.setLayout(server_layout)
            layout.addWidget(server_group)

            files_group = QGroupBox("📂 Архивация файлов")
            files_group.setStyleSheet(AppStyles.group_box())
            ...
            files_info = QLabel(
                ...
                <tr><td><b>Папка файлов:</b></td><td>{files_path}</td></tr>
                <tr><td><b>Доступность:</b></td><td>{'✅ Доступна' if files_exists else '❌ Не найдена'}</td></tr>
                </table>
                """
            )
            files_info.setStyleSheet(AppStyles.lable())
            files_info.setTextFormat(Qt.RichText)
            files_layout.addWidget(files_info)

            self.backup_include_files = QCheckBox("📁 Включить прикрепленные документы")
            self.backup_include_files.setChecked(True)
            self.backup_include_files.setStyleSheet(AppStyles.check())
            files_layout.addWidget(self.backup_include_files)
            ...
            files_layout.addWidget(self.backup_include_exe)

            hint_label = QLabel("   ℹ️ В архив будет добавлен файл метаданных SQL Server и выбранные файлы.")
            hint_label.setStyleSheet(AppStyles.lable())
            files_layout.addWidget(hint_label)
        ...
        self.tab_widget.currentChanged.connect(self._update_primary_action_text)
        self._update_primary_action_text()

    def _update_primary_action_text(self):
        if self.tab_widget.currentIndex() == 0:
            self.export_btn.setText("✅ Экспортировать")
            return
        if self._is_sqlserver_backend():
            self.export_btn.setText("📦 Архивировать файлы")
        else:
            self.export_btn.setText("📦 Архивировать")
    ...
    def get_documents_for_period(self) -> List[Dict]:
        ...
        try:
            ...
            if self.single_day_radio.isChecked():
                ...
                query = """
                    SELECT d.* FROM documents d
                    WHERE date(d.reg_date) BETWEEN date(?) AND date(?)
                    ORDER BY d.reg_date, d.id
                """
                ...
                # Все документы
                query = "SELECT d.id FROM documents d ORDER BY d.reg_date DESC, d.id DESC"
                params = ()

            if hasattr(self.db_manager, "get_documents_for_period"):
                if self.single_day_radio.isChecked():
                    return self.db_manager.get_documents_for_period(selected_date, selected_date)
                if self.date_range_radio.isChecked():
                    return self.db_manager.get_documents_for_period(date_from, date_to)
                return self.db_manager.get_documents_for_period()
    ...
    def create_backup(self):
        ...
        try:
            if getattr(self.db_manager, "backend", "sqlite") == "sqlserver":
                self._create_sqlserver_files_archive()
                return
                QMessageBox.information(
                    self,
                    "Серверная база данных",
                    "Для профиля SQL Server серверный backup базы выполняется отдельно на сервере.\n\n"
                    "В клиенте доступны экспорт данных и экспорт файлов.",
                )
                return
            ...
            # Для WAL-режима принудительно сбрасываем изменения в основной .db,
            # чтобы архив содержал консистентное состояние базы.
            try:
                mode_row = self.db_manager.connection.execute("PRAGMA journal_mode").fetchone()
                current_mode = (mode_row[0] if mode_row else "").lower()
                if current_mode == "wal":
                    self.db_manager.connection.execute("PRAGMA wal_checkpoint(FULL)")
            except Exception as checkpoint_error:
                print(f"⚠️ Не удалось выполнить WAL checkpoint: {checkpoint_error}")
            ...
            # Добавляем вспомогательные WAL/SHM файлы, если они существуют.
            for sqlite_aux_file in (f"{db_file}-wal", f"{db_file}-shm"):
                if os.path.exists(sqlite_aux_file):
                    files_to_archive.append((sqlite_aux_file, os.path.basename(sqlite_aux_file)))
            ...
            with tarfile.open(archive_path, "w:gz") as tar:
                for idx, (filepath, arcname) in enumerate(files_to_archive):
                    ...
                    # Обновляем прогресс
                    progress_value = 20 + int(((idx + 1) / total_files) * 70)
                    progress.setValue(progress_value)
    ...
    def export_database(self):
        ...
        try:
            ...
            if self._is_sqlserver_backend():
                progress.setValue(10)
                progress.setLabelText("Выгрузка метаданных SQL Server...")
                exported = self._write_sqlserver_export_manifest(
                    export_folder,
                    include_files=self.include_files_checkbox.isChecked(),
                )
                stats = self.db_manager.get_documents_statistics()
                progress.setValue(100)
                progress.close()
                ...
                QMessageBox.information(
                    self,
                    "✅ Экспорт завершен",
                    f"Экспорт SQL Server успешно выполнен:\n\n"
                    f"{export_folder}\n\n"
                    f"Метаданные: {os.path.basename(exported['manifest_path'])}\n"
        ...
        try:
            if Workbook is None:
                QMessageBox.critical(
                    parent_widget,
                    "Ошибка экспорта",
                    f"Модуль openpyxl недоступен: {_OPENPYXL_IMPORT_ERROR}\n\n"
                    "Запустите полную папку сборки (exe + _internal) или пересоберите приложение."
                )
                return None
