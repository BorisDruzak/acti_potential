...
from PyQt5.QtCore import Qt, QTimer, QDate, pyqtSignal, QPropertyAnimation, QEasingCurve, QRect, QPoint, QSize, QModelIndex
from PyQt5.QtGui import QFont
from documents_table import DocumentsTableModel
from document_utils import format_date_for_display, parse_flexible_datetime
...
class AdvancedSearchDialog(QDialog):
    ...
    def do_search(self):
        ...
        try:
            ...
            params = []
            date_expr = (
                self.db_manager._document_date_expr()
                if hasattr(self.db_manager, "_document_date_expr")
                else "date(d.reg_date)"
            )
            date_param_expr = (
                "TRY_CONVERT(date, ?, 23)"
                if getattr(self.db_manager, "backend", "") == "sqlserver"
                else "date(?)"
            )
            ...
            if 'date_from' in self.active_filters:
                query += f" AND {date_expr} >= {date_param_expr}"
            ...
            query += f" ORDER BY {date_expr} DESC, d.id DESC LIMIT 1000"
...
class SearchResultsTableModel(DocumentsTableModel):
    """Model for the advanced search table where date/number columns differ."""

    DATE_COLUMNS = {4}
    NUMERIC_COLUMNS = {0}
    NATURAL_TEXT_COLUMNS = {2, 3, 14, 15}
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role in (Qt.DisplayRole, Qt.EditRole):
            if col in self.DATE_COLUMNS and value:
        ...
        try:
            def get_key(row):
                ...
                if column in self.DATE_COLUMNS:
                    parsed_date = parse_flexible_datetime(value)
                    return (0, parsed_date.timestamp() if parsed_date else 0)

                if column in self.NATURAL_TEXT_COLUMNS:
                    return (0, self._natural_key(value))

                return (0, str(value).lower())

            self._data.sort(key=get_key, reverse=order == Qt.DescendingOrder)
            self._sort_column = column
            self._sort_order = order
        except Exception as e:
            print(f"Ошибка сортировки результатов поиска: {e}")
        finally:
...
class SearchResultsTableView(QTableView):
    ...
    def update_with_documents(self, documents):
        ...
        try:
            ...
            # Создаем модель
            self.model_instance = SearchResultsTableModel(table_data, headers)
    ...
    def contextMenuEvent(self, event):
        ...
        try:
            ...
            # Если кликнули по невыделенной строке, выделяем только её.
            row = index.row()
            if self.selectionModel() and not self.selectionModel().isRowSelected(row, QModelIndex()):
                self.clearSelection()
                self.selectRow(row)
            ...
            selected_count = len(self.selectionModel().selectedRows()) if self.selectionModel() else 0
            ...
            export_menu = menu.addMenu("📤 Экспортировать документы")
            export_selected_action = QAction(
                f"📤 Экспортировать выделенное ({selected_count})",
                self
            )
            export_selected_action.triggered.connect(self.export_selected_documents)
            
            export_all_action = QAction("📤 Экспортировать всё", self)
            export_all_action.triggered.connect(self.export_all_documents)
    ...
    def _get_selected_document_ids(self):
        """Получить ID выбранных документов."""
        ...
        for model_index in self.selectionModel().selectedRows():
            row = model_index.row()
            doc_ids.append(self.model()._data[row][0])
        return doc_ids
    
    def _get_all_document_ids(self):
        """Получить ID всех документов в текущем результате поиска."""
        if not hasattr(self, 'model_instance') or not self.model_instance:
            return []
        return [row[0] for row in self.model_instance._data]
    
    def _open_export_dialog_for_ids(self, doc_ids, mode):
        """Открыть диалог экспорта для списка ID документов."""
        documents = []
        ...
        if not documents:
            QMessageBox.warning(
                self,
                "⚠️ Нет данных",
                "Не удалось получить документы для экспорта"
            )
            return
        
        from export_manager import ExportDialog
        dialog = ExportDialog(self.db_manager, self)
        dialog.set_documents_for_export(documents, mode=mode)
        dialog.exec_()
    
    def export_selected_documents(self):
        """Экспорт выделенных документов из результатов поиска."""
        doc_ids = self._get_selected_document_ids()
        if not doc_ids:
            QMessageBox.warning(
                self,
                "⚠️ Нет выделения",
                "Выделите документы для экспорта"
            )
            return
        
        self._open_export_dialog_for_ids(doc_ids, mode='selected')
    
    def export_all_documents(self):
        """Экспорт всех документов, найденных в текущем поиске."""
        ...
        self._open_export_dialog_for_ids(doc_ids, mode='all_from_table')
