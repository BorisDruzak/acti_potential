...
from document_utils import format_date_for_display, parse_flexible_datetime, to_iso_date
import re
...
class DocumentsTableModel(QAbstractTableModel):
    ...
    def __init__(self, data, headers):
        ...
        self.editable_columns = {
            ...
            'reg_date': self.DATE_COLUMN,
            'status': self.STATUS_COLUMN,
            'type_doc': self.TYPE_COLUMN,
        }
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole or role == Qt.EditRole:
            # Форматирование даты
            if col == self.DATE_COLUMN and value:
            ...
            if col == self.TITLE_COLUMN:
                font.setPointSize(9)
                font.setWeight(QFont.Medium)
            elif col == self.ID_COLUMN:
                font.setBold(True)
                font.setPointSize(8)
            return font
            ...
            elif col in [self.REG_NUMBER_COLUMN, self.DATE_COLUMN]:
                return Qt.AlignCenter
            return Qt.AlignLeft | Qt.AlignVCenter
        
        elif role == Qt.ToolTipRole:
            # Детальные подсказки
            if col == self.TITLE_COLUMN and isinstance(value, str) and len(value) > 40:
                return f"<b>Полное название:</b><br>{value}"
            elif col == self.STATUS_COLUMN:
                return f"<b>Текущий статус документа:</b><br>{value}"
            elif col == self.EXECUTOR_COLUMN:
                return f"<b>Ответственный исполнитель:</b><br>{value}"
            elif col == self.THEME_COLUMN:
        ...
        if col == self.DATE_COLUMN:
            try:
                if isinstance(value, str) and value.strip():
                    normalized_date = to_iso_date(value.strip())
                    if normalized_date:
    ...
    def _sort_value(self, value, column):
        ...
        if column == self.REG_NUMBER_COLUMN:
    ...
    def _apply_sort_to_data(self):
        if self._sort_column is None or not self._data:
            return
        ...
        for row in self._data:
            ...
            if self._is_empty_sort_value(value):
                empty_rows.append(row)
            else:
                non_empty_rows.append(row)

        non_empty_rows.sort(
            key=lambda row: self._sort_value(row[column], column),
            reverse=reverse,
        )
        self._data[:] = non_empty_rows + empty_rows
...
class DocumentsTableView(QTableView):
    ...
    @staticmethod
    def _format_documents_for_table(documents):
        table_data = []
        for doc in documents:
            table_data.append([
                ...
                doc.get('reg_date') or "-",
                doc.get('status_name') or doc.get('status') or "Не указан",
                doc.get('type_name') or doc.get('type_doc') or "Не указан",
    ...
    def refresh_loaded_documents(self):
        """
        Refresh the already loaded window without resetting pagination or scroll.

        This is intended for background sync: explicit user filters still use
        load_documents(reset=True), but live refresh keeps the current loaded
        amount so the counter does not jump back to the first 100 rows.
        """
        if self._is_loading:
            return
        ...
        loaded_count = len(self._all_loaded_data)
        pages_to_refresh = max(
            1,
            self.current_page,
            (loaded_count + self.per_page - 1) // self.per_page,
        )
        refresh_limit = max(self.per_page, pages_to_refresh * self.per_page)

        try:
            documents, total_count, _has_more = self.db_manager.get_documents_paginated(
                page=1,
                per_page=refresh_limit,
                filters=self.current_filters,
            )

            self._all_loaded_data = self._format_documents_for_table(documents)
            self.total_documents = total_count
            self.current_page = max(
                1,
                (len(self._all_loaded_data) + self.per_page - 1) // self.per_page,
            )
            self.has_more = len(self._all_loaded_data) < self.total_documents
            ...
            QTimer.singleShot(
                0,
                lambda: scrollbar.setValue(min(previous_scroll_value, scrollbar.maximum())),
            )
            print(
                f"✅ Обновлено без сброса: "
                f"{len(self._all_loaded_data)}/{self.total_documents}"
            )
        except Exception as e:
            print(f"❌ Ошибка фонового обновления таблицы: {e}")
            import traceback
            traceback.print_exc()
        finally:
            self._is_loading = False
